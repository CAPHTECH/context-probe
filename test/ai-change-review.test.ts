...
describe("ai change review scoring", () => {
  test("requires base and head branches for ai_change_review", async () => {
  ...
  test("fails clearly when a requested branch does not exist", async () => {
    const fixture = await createAiChangeReviewFixture();
    try {
      await expect(
        COMMANDS["score.compute"]!(
  ...
  test("returns an empty review target list for an empty branch diff", async () => {
  ...
  test("scores changed files and emits review targets with expected reasons", async () => {
    const fixture = await createAiChangeReviewFixture();
    try {
      const response = await COMMANDS["score.compute"]!(
        {
