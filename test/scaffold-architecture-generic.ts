...
export function registerScaffoldArchitectureGenericTests(tempRoots: string[]): void {
  test("constraints.scaffold splits broad infrastructure buckets using generic file-role heuristics", async () => {
    ...
    expect(layers.find((layer) => layer.name === "RuntimeInfrastructure")?.globs).toEqual(["src/adapters/duckdb.ts"]);
    expect(layers.find((layer) => layer.name === "WorkspaceBootstrap")?.globs).toEqual(["src/project/workspace.ts"]);
  });
}
