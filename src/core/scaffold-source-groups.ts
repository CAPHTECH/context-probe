...
function mergeSourceGroupCollection(key: string, groups: SourceGroup[]): SourceGroup {
  ...
  return {
    ...
    heuristicSplit: groups.some((group) => group.heuristicSplit) || origins.length > 1,
...
function mergeMonorepoEquivalentGroups(groups: SourceGroup[]): SourceGroup[] {
  ...
  const merged = Array.from(mergeableByDisplayName.entries()).map(([normalizedDisplayName, matchingGroups]) => {
    ...
    const mergedGroup = mergeSourceGroupCollection(`__merged__/${normalizedDisplayName}`, matchingGroups);
    return {
      ...mergedGroup,
